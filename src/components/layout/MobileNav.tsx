import React from 'react';
import { NavLink } from 'react-router-dom';
import { Home, TrendingUp, Wallet, CreditCard, User } from 'lucide-react';
import { useLanguage } from '../../contexts/LanguageContext';
import { brandConfig } from '../../config/brand';

const navItems = [
  { to: '/dashboard', icon: Home, label: 'dashboard' },
  { to: '/invest', icon: TrendingUp, label: 'invest' },
  { to: '/deposit', icon: Wallet, label: 'deposit' },
  { to: '/withdraw', icon: CreditCard, label: 'withdraw' },
  { to: '/profile', icon: User, label: 'profile' }
];

export function MobileNav() {
  const { t } = useLanguage();

  return (
    <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-gray-200 z-50 shadow-2xl">
      <div className="grid grid-cols-5 h-20 px-2">
        {navItems.map((item) => (
          <NavLink
            key={item.to}
            to={item.to}
            className={({ isActive }) => `
              flex flex-col items-center justify-center p-3 transition-all duration-300 rounded-2xl mx-1 my-2
              ${isActive
                ? `text-white bg-gradient-to-br from-[${brandConfig.colors.primary}] to-[${brandConfig.colors.primary}]/80 shadow-lg transform scale-105`
                : 'text-gray-500 hover:text-gray-700 hover:bg-gray-100 active:scale-95'
              }
            `}
          >
            <item.icon className="h-6 w-6 mb-1" />
            <span className="text-xs font-semibold">{t(item.label)}</span>
          </NavLink>
        ))}
      </div>
    </nav>
  );
}